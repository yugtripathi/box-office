...
export async function MovieLiveCounter({ movieId }: MovieLiveCounterProps) {
    ...
    return (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/20 via-card to-card border border-primary/30 p-6">
            ...
            <div className="absolute -bottom-24 -left-24 w-48 h-48 bg-primary/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
            ...
            <div className="relative grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {/* Total Collection */}
                <div className="col-span-1 md:col-span-2 p-5 rounded-xl bg-gradient-to-r from-primary/10 to-transparent border border-primary/20">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                        <IndianRupee className="h-4 w-4" />
                        <span>Total Collection</span>
                    </div>
                    <div className="text-4xl md:text-5xl font-black text-primary tabular-nums tracking-tight">
            ...
            {/* Latest Day Card */}
            <div className="relative flex items-center justify-between p-4 rounded-xl bg-muted/20 border border-border/50">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <TrendingUp className="h-4 w-4 text-[hsl(142,71%,45%)]" />
                        <span className="text-sm font-medium">Day {latestDayNumber}</span>
                    </div>
                    <div className="text-xl font-bold text-[hsl(142,71%,45%)]">{formatINR(latestDay.amount)}</div>
                </div>
